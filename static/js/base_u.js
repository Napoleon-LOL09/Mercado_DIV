    ...
    //Registro exitoso - Base.html
        ...
        function mostrarResultados(respuesta) {
            ...
            for (var i = 0; i < respuesta.length; i++) {
                html += '<div>';
                html += '<a class="ml-2 mb-2" style="font-size: 19px" href="/User_v/ver_productos.html/' + respuesta[i].id + '">' + respuesta[i].name + '</a>';
            }
            $('#Productos').html(html);
        ...
        $(".carousel-item .Imgs_p").each(function(index) {
            let value = $('.lasimg').eq(index).val(); // Obtener el valor de la imagen oculta
            $(this).attr('src', '../../../static/' + value); // Asignar la ruta de la imagen al atributo src de cada elemento <img>
        });
