$(document).ready(function () {
    ...
    if ((error1 =  $('#error1').text()) != ""){
        error (error1, $("#error1"))
    } else if ((error2 =  $('#error2').text()) != "") {
        error (error2, $("#error2"))
    }

    function error (error, error_c) {
        if (error != ""){
            $(error_c).fadeOut(4000, function() {
                window.location.href = "/Admin_v/productos.html"
            })
        }
    }
            ...
            success: function(response) {
                ...
                if (resultados != "") {
                    productos_filtrado.forEach(function(product) {
                        ...
                        html += '<a href="/Admin_v/ver_productos.html/' + product.id + '" style="font-size: 20px;" >' + product.name + '</a>' ;
                        html += '</div>';
                    });
                    $('#result_fil').show();
